function PopDec = EGOSelect2(Problem,Population,L1,L2,Ke,delta,nr)
...
% This function is written by Cheng He
    ...
    PopDec  = cat(1,NewPop.dec);
    PopObj = cat(1,NewPop.obj);
    gmin    = inf;
    while (maxeva>0)
       for i = 1 : N
           if rand < delta
           ...
           OffDec = OperatorDE2(PopDec(i,:),PopDec(P(1),:),PopDec(P(2),:),Problem);
...
% Calculate the mu (x) and sigma^2 (y) of the aggregation function
    ...
    x     = abs((mu(1)^2+lamda(1)^2*sig2(1))*normcdf(alpha) + (mu(2)^2+lamda(2)^2*sig2(2))*normcdf(-alpha) + sum(mu)*normpdf(alpha) - y.^2);
end
